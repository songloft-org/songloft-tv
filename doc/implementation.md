...
- **播放器控制栏常驻**：`player_controls_persistent`（默认关）。开启后全屏播放器底部功能菜单不再 10 秒自动隐藏（K 歌模式同样生效）；返回键/点击空白手动隐藏不受影响。音效面板的自动收起不受此设置控制。
...
- **版权与免责声明**：首次进入主界面弹窗展示（`disclaimer_shown` DataStore 标记，任意方式关闭即视为已展示不再弹出），含版权说明/免责声明/隐私说明/使用安全声明（严禁驾驶中操作本应用的车载警示）四个板块；底部「操作说明」按钮先关闭免责声明再打开 `HelpDialog`，返回键只回到主界面、不会回到免责声明。
